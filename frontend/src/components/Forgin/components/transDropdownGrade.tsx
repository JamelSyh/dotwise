...
import { setInLang, setOutLang } from "app/translator/translator";
  ...
  const populateOptions = () => {
    ...
    return opt[1].map((option, index) => (
      <div key={index} className={`option ${grade.code === option.code ? " active" : ""}`} onClick={() => {
        ...
        const updatedArray = [updatedElement, ...lang.slice(1)];

        if (id == "in") {
          dispatch(setInLang(updatedArray));
        } else {
          dispatch(setOutLang(updatedArray));
        }
        setActive(false);
